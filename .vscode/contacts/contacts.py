class Contact:
    def __init__(self, name, phone, email, addr):
        self.name = name # 서로 다른 name이고, 할당함으로써 연결
        self.phone = phone
        self.email = email
        self.addr = addr
    ...
    def set_contact():
        ...
        email = input('이메일')
        addr = input('주소')
        contact = Contact(name, phone, email, addr) # Contact는 클래스명, contact는 인스턴스명
        return contact

    @staticmethod
    def get_contact(clist):
        for i in clist:
            i.print_info() # 8번 라인에 선언된 메소드(클래스 내부), 함수는 클래스 밖에 있다.

    @staticmethod
    def del_contact(clist, name):
        for i, t in enumerate(clist): # i는 인덱스, t는 요소를 출력한다.
            if t.name == name:
                del clist[i]
